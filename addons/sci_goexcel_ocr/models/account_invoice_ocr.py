...
class AccountInvoice(models.Model):
    ...
    def get_bl_no(self, partner_template, text_file_dic):
        ...
        for line in text_file:
            ...
            if check_bl_no:
                break
        if partner_template.name == "YANG MING LINE (M) SDN BHD":
            if bl_no and bl_no[0] == '1':
                bl_no = bl_no.replace(bl_no[0], 'I', 1)
        return bl_no
    ...
    def get_payment_term(self, partner_template, text_file_dic, date, due_date):
        ...
        for line in text_file:
            if len(line) > 3 and not check_payment_term:
                mappings = payment_term_keywords.mapping_line_ids
                if partner_template.name == 'SEALAND':
                    mappings = partner_template.payment_term_template_label
    ...
    def get_product_mapping_lines(self, mapping_id, keyword, direction, container_size, partner_id):
        mapping_lines = self.env['ocr.table.mapping.line'].search([
                    ...
                    if partner_template.complex_line:
                        ...
                        for temp_name in temp_names:
                            ...
                            if not check_price:
                                ...
                                if not product:
                                    product_ids = self.get_product_mapping_lines(product_keywords.id, product_name,
                                                                                 direction, container_size,
                                                                                 partner_template.partner_id.id)
                        ...
                        if partner_template.name == 'MEDITERRANEAN SHIPPING COMPANY (M) SDN BHD':
                            line = line.replace('1x', '1 x').replace('x BL', 'x 40HC')
                        line_with_price = line.replace(',', '')
                        line_with_price = line_with_price.lstrip('0123456789.- ')
                        price_list_raw = re.findall('\d+\.\d{2,6}|\d+', line_with_price)
                        price_list = price_list_raw[-len(line_item_pattern.split()):]
                        if len(price_list) == 5 and partner_template.name == 'MEDITERRANEAN SHIPPING COMPANY (M) SDN BHD':
                            try:
                                price_list.insert(2, str(float(price_list[0]) * float(price_list[2])))
                            except:
                                print('not floatable >>> ', line, '|', price_list[0], '|', price_list[2])
                                pass
                        ...
                        if 0 < len(price_list) == x:
                            ...
                            if "TOTAL" in line_item_pattern:
                                # Get Currency in Line
                                if partner_template.name in ['MEDITERRANEAN SHIPPING COMPANY (IDR)',
                                                             'MEDITERRANEAN SHIPPING COMPANY (USD)']:
                                ...
                                for temp_name in temp_names:
                                    if not temp_name.isdecimal() and (
                                            len(temp_name) > 2 or (
                                            partner_template.name == 'MEDITERRANEAN SHIPPING COMPANY (M) SDN BHD' and temp_name == 'BL')
                                    ):
                                        if '.' not in temp_name:
                                            # Get Product Name
                                            if not product_name:
                                            ...
                                            if not product:
                                                product_ids = self.get_product_mapping_lines(product_keywords.id,
                                                                                             product_name, direction,
                                                                                             container_size,
                                                                                             partner_template.partner_id.id)
                                                # print(product_ids)
                                                if len(product_ids) == 1:
                                                    ...
                                                    for prod in product_ids:
                                                        if prod.name in line:
                                                            product = prod
                                                            temp_flag = False
                                                            break
                                                    if temp_flag:
                                                        product_ids = self.get_product_mapping_lines(
                                                            product_keywords.id, product_name, direction,
                                                            container_size, partner_template.partner_id.id)
                                                        if len(product_ids) == 1:
                                                            product = product_ids[0]
                                                else:
                                                    product_name = False
                    ...
                    if x == len(price_list) and not product_excluded:
                        if quantity < 1000:
                            if partner_template.name in ['MEDITERRANEAN SHIPPING COMPANY (IDR)',
                                ...
                                if partner_template.name in ['YANG MING LINE (M) SDN BHD', 'Orient Overseas Container Line', 'SEALAND' ,'COSCO SHIPPING Lines (Malaysia) Sdn. Bhd.']:
                                    amount_total = price * quantity
                                else:
                                    amount_total = price * quantity * currency_rate
                                line_amount_total = float_round(amount_total, 2, rounding_method='HALF-UP') or 0.0
                            if product:

                                dic = {
                                    'product_id': product.product_id.id or False,
                                    # 'name': f'{product.product_id.name} ({product.keyword})' or False,
                                    'name': product.keyword or False,
        ...
        for line in text_file:
            if len(line) > 3:
                if not container_size:
                    if '40HQ' in line or '40GP' in line or '40\'' in line or '40/' in line or '40H' in line or '40!' in line or '40HC' in line or '40\'HQ' in line or '40/9\'6/DRY' in line:
                        container_size = 40
                    if '20HQ' in line or '20GP' in line or '20\'' in line or '20/' in line or '20H' in line or '20!' in line or '20ST' in line or '20/8\'6/DRY' in line or '20DV' in line or '20FT' in line or '20DC' in line:
                        container_size = 20
