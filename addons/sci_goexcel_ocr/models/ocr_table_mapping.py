...
class OCRTableMappingLine(models.Model):
    ...
    def create_data(self):
        lines = self.sudo().search([('line_id', '=', 10), ('company_id', '=', 1)])
        data = dict([
            ("MEDITERRANEAN SHIPPING COMPANY (M) SDNBHD", 17495),
            ("CMA CGM MALAYSIA SDN BHD", 16305),
            ("COSCO SHIPPING LINES (MALAYSIA) SDN BHD", 16320),
            ("WAN HAI LINES (M) SDN BHD", 16476),
            ("YANG MING LINE (M) SDN BHD", 16482),
            ("MAERSK LOGISTICS & SERVICES MALAYSIA SDN BHD", 16455),
            ("SEALAND MAERSK ASIA PTE LTD ON BEHALFMAERSK MALAYSIA SDN BHD", 16455),
            ("MEDITERRANEAN SHIPPING CO (M) SDN BHD", 17098),
            ("ORIENT OVERSEAS CONTAINER LINE (M) SDN BHD", 16417)
        ])
        for line in lines:
            product_line = self.search([
                ('keyword', '=', line.keyword),
                ('partner_id', '=', line.partner_id.id),
                ('company_id', '=', 2)
            ])
            product = self.env['product.product'].search([('name', '=', product_line.sudo().product_id.name)], limit=1)
            line.write({
                'partner_id': data.get(line.partner_id.name, False),
...
class OCRPartnerTemplate(models.Model):
    ...
    def _onchange_partner_id(self):
        ...
        if self.partner_id:
            self.partner_id.ocr_partner_template = self.id