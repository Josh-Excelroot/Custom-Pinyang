...
class KsDashboardNinjaItemAdvance(models.Model):
    ...
    def ks_run_query(self):
        ...
        if self.ks_query_end_date:
            if self.ks_custom_query and (("%(ks_start_date)" in self.ks_custom_query) or ("%(ks_end_date)" in self.ks_custom_query)):
                ks_type = 'date'
                ks_start_date = 'ks_start_date'
                ks_end_date = 'ks_end_date'
            if self.ks_custom_query and (("%(ks_start_datetime)" in self.ks_custom_query) or ("%(ks_end_datetime)" in self.ks_custom_query)):
                ks_type = 'datetime'
    ...
    def ks_get_kpi_result(self,ks_query, selected_start_date, selected_end_date, ks_start_date=False, ks_end_date=False):
