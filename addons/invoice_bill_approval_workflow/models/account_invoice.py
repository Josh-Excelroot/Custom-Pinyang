...
class AccountInvoice(models.Model):
    ...
    # Ahmad Zaman - 19/10/24 - Invoice Approval Settings Changes
    is_invoice_approver = fields.Boolean("Is Invoice Approver", compute="_compute_is_invoice_approver", store=False)

    @api.depends('is_invoice_approver')
    def _compute_is_invoice_approver(self):
        current_user = self.env.user
        invoice_approver = current_user.company_id.invoice_user_ids.ids
        for record in self:
            record.is_invoice_approver = invoice_approver
    # end
    ...
    def action_invoice_open_ip(self):
        ...
        if self.company_id.invoice_bill_approval or self.company_id.customer_vendor_credit_approval:
            ...
            if self.type == 'out_invoice':
                ...
                list_of_user = self.company_id.invoice_notification_user_ids
                ...
                list_of_user = self.company_id.bill_notification_user_ids
                ctx['type'] = 'Bill'
    ...
    def action_invoice_reject(self):
        ...
        if wiz_id:
            return {
                ...
                'views': [(self.env.ref('invoice_bill_approval_workflow.reject_reason_view_form').id, 'form')],
                'view_id': self.env.ref('invoice_bill_approval_workflow.reject_reason_view_form').id,
                'target': 'new',
                'res_id': wiz_id.id,
            }
        #self.write({'state': 'draft'})
