...
class InsGeneralLedgerXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, filter):
        ...
        if filter:
            ...
            self.sheet_2.write_string(self.row_pos_2, 0, _('Analytic Tags'),
                                      self.format_header)
            a_list = ', '.join(
                [lt or '' for lt in filter.get('analytic_tags')])
            self.sheet_2.write_string(self.row_pos_2, 1, a_list,
                                      self.content_header)
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        if filter.get('include_details', False):
            ...
            self.sheet.merge_range(
                self.row_pos, 0, self.row_pos, 1, _('Code'), self.format_header)
            self.sheet.merge_range(self.row_pos, 2, self.row_pos, 4, _(
                'Account'), self.format_header)
            self.sheet.write_string(self.row_pos, 5, _('Debit'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 6, _('Credit'),
                                    self.format_header)
        ...
        if acc_lines:
            for line in acc_lines:
                self.row_pos += 1
                self.sheet.merge_range(self.row_pos, 0, self.row_pos, 4, '            ' + acc_lines[line].get(
                    'code') + ' - ' + acc_lines[line].get('name'), self.line_header_left)
                self.sheet.write_number(self.row_pos, 5, float(
                    acc_lines[line].get('debit')), self.line_header)
                self.sheet.write_number(self.row_pos, 6, float(
                    acc_lines[line].get('credit')), self.line_header)
                self.sheet.write_number(self.row_pos, 7, float(
                    acc_lines[line].get('balance')), self.line_header)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        if record:
            data = record.read()
            self.sheet.merge_range(
                0, 0, 0, 8, 'General Ledger'+' - '+data[0]['company_id'][1], self.format_title)
