...
class InsTrialBalanceXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, acc_lines, retained, subtotal, filter):
        ...
        self.row_pos += 3
        self.sheet.merge_range(self.row_pos, 1, self.row_pos,
                               3, 'Initial Balance', self.format_merged_header)
        ...
        self.sheet.merge_range(self.row_pos, 7, self.row_pos,
                               9, 'Ending Balance', self.format_merged_header)
        ...
        if acc_lines:
            if not filter.get('show_hierarchy'):
                for line in acc_lines:  # Normal lines
                    self.row_pos += 1
                    self.sheet.write_string(self.row_pos, 0,  acc_lines[line].get(
                        'code') + ' ' + acc_lines[line].get('name'), self.line_header_light_left)
                    self.sheet.write_number(self.row_pos, 1, float(
                        acc_lines[line].get('initial_debit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 2, float(
                        acc_lines[line].get('initial_credit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 3, float(
                        acc_lines[line].get('initial_balance')), self.line_header_highlight)
                    self.sheet.write_number(self.row_pos, 4, float(
                        acc_lines[line].get('debit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 5, float(
                        acc_lines[line].get('credit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 6, float(
                        acc_lines[line].get('balance')), self.line_header_highlight)
                    self.sheet.write_number(self.row_pos, 7, float(
                        acc_lines[line].get('ending_debit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 8, float(
                        acc_lines[line].get('ending_credit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 9, float(
                        acc_lines[line].get('ending_balance')), self.line_header_highlight)
            else:
                for line in acc_lines:  # Normal lines
                    self.row_pos += 1
                    ...
                    if line.get('dummy'):
                        ...
                        self.sheet.write_string(self.row_pos, 0, blank_space + line.get('code') + ' ' + line.get('name'),
                                                self.line_header_light_left)
                    self.sheet.write_number(self.row_pos, 1, float(
                        line.get('initial_debit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 2, float(
                        line.get('initial_credit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 3, float(
                        line.get('initial_balance')), self.line_header_highlight)
                    self.sheet.write_number(self.row_pos, 4, float(
                        line.get('debit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 5, float(
                        line.get('credit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 6, float(
                        line.get('balance')), self.line_header_highlight)
                    self.sheet.write_number(self.row_pos, 7, float(
                        line.get('ending_debit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 8, float(
                        line.get('ending_credit')), self.line_header_light)
                    self.sheet.write_number(self.row_pos, 9, float(
                        line.get('ending_balance')), self.line_header_highlight)

            if filter.get('strict_range'):
                # Retained Earnings line
                self.row_pos += 1
                self.sheet.write_string(
                    self.row_pos, 0, '        ' + retained['RETAINED'].get('name'), self.line_header_light_left)
                self.sheet.write_number(self.row_pos, 1, float(
                    retained['RETAINED'].get('initial_debit')), self.line_header_light)
                self.sheet.write_number(self.row_pos, 2, float(
                    retained['RETAINED'].get('initial_credit')), self.line_header_light)
                self.sheet.write_number(self.row_pos, 3, float(
                    retained['RETAINED'].get('initial_balance')), self.line_header_highlight)
                self.sheet.write_number(self.row_pos, 4, float(
                    retained['RETAINED'].get('debit')), self.line_header_light)
                self.sheet.write_number(self.row_pos, 5, float(
                    retained['RETAINED'].get('credit')), self.line_header_light)
                self.sheet.write_number(self.row_pos, 6, float(
                    retained['RETAINED'].get('balance')), self.line_header_highlight)
                self.sheet.write_number(self.row_pos, 7, float(
                    retained['RETAINED'].get('ending_debit')), self.line_header_light)
                self.sheet.write_number(self.row_pos, 8, float(
                    retained['RETAINED'].get('ending_credit')), self.line_header_light)
                self.sheet.write_number(self.row_pos, 9, float(
                    retained['RETAINED'].get('ending_balance')), self.line_header_highlight)
            # Sub total line
            self.row_pos += 2
            self.sheet.write_string(self.row_pos, 0,  subtotal['SUBTOTAL'].get(
                'code') + ' ' + subtotal['SUBTOTAL'].get('name'), self.line_header_left_total)
            self.sheet.write_number(self.row_pos, 1, float(
                subtotal['SUBTOTAL'].get('initial_debit')), self.line_header_light_total)
            self.sheet.write_number(self.row_pos, 2, float(
                subtotal['SUBTOTAL'].get('initial_credit')), self.line_header_light_total)
            self.sheet.write_number(self.row_pos, 3, float(
                subtotal['SUBTOTAL'].get('initial_balance')), self.line_header_total)
            self.sheet.write_number(self.row_pos, 4, float(
                subtotal['SUBTOTAL'].get('debit')), self.line_header_light_total)
            self.sheet.write_number(self.row_pos, 5, float(
                subtotal['SUBTOTAL'].get('credit')), self.line_header_light_total)
            self.sheet.write_number(self.row_pos, 6, float(
                subtotal['SUBTOTAL'].get('balance')), self.line_header_total)
            self.sheet.write_number(self.row_pos, 7, float(
                subtotal['SUBTOTAL'].get('ending_debit')), self.line_header_light_total)
            self.sheet.write_number(self.row_pos, 8, float(
                subtotal['SUBTOTAL'].get('ending_credit')), self.line_header_light_total)
            self.sheet.write_number(self.row_pos, 9, float(
                subtotal['SUBTOTAL'].get('ending_balance')), self.line_header_total)
    ...
    def convert_to_date(self, datestring=False):
        if datestring:
            datestring = fields.Date.from_string(
                datestring).strftime(self.language_id.date_format)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.language_id = self.env['res.lang'].search(
            [('code', '=', lang)])[0]
        ...
        if record:
            ...
            self.sheet.merge_range(
                0, 0, 0, 10, 'Trial Balance'+' - '+data[0]['company_id'][1], self.format_title)
            ...
            # Content section
            self.prepare_report_contents(
                account_lines, retained, subtotal, filters)
