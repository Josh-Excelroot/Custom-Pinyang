...
class InsPartnerAgeingXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, filter):
        ...
        if filter:
            ...
            p_list = filter.get("salesperson") or ""
            self.sheet_2.write_string(self.row_pos_2, 1, p_list, self.content_header)
    ...
    def prepare_report_contents(
        ...
        if self.record.include_details:
            ...
            self.sheet.merge_range(
                self.row_pos, 0, self.row_pos, 3, _("Partner"), self.format_header
            )
        k = 4
        ...
        if ageing_lines:
            for line in ageing_lines:
                ...
                self.row_pos += 1
                self.sheet.write_string(
                    self.row_pos, 4, "", self.line_header_light_period
                )
                self.sheet.write_string(
                    self.row_pos, 5, "", self.line_header_light_period
                )
                self.sheet.write_string(
                    ...
                    if line != "Total":
                        ...
                        for sub_line in sub_lines:
                            ...
                            date = self.convert_to_date(
                                sub_line.get("date_maturity")
                                and sub_line.get("date_maturity").strftime("%Y-%m-%d")
                                or sub_line.get("date").strftime("%Y-%m-%d")
                            )
                            self.sheet.write_datetime(
                                self.row_pos, 1, date, self.line_header_light_date
                            )
                            self.sheet.write_string(
                                self.row_pos,
                                2,
