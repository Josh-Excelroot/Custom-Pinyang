...
class InsPartnerLedgerXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        if filter.get('include_details', False):
            ...
            self.sheet.merge_range(self.row_pos, 0, self.row_pos, 4, _(
                'Partner'), self.format_header)
            self.sheet.write_string(self.row_pos, 5, _('Debit'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 6, _('Credit'),
                                    self.format_header)
        ...
        if acc_lines:
            for line in acc_lines:
                ...
                self.sheet.merge_range(
                    self.row_pos, 0, self.row_pos, 4, acc_lines[line].get('name'), self.line_header)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        if record:
            data = record.read()
            self.sheet.merge_range(
                0, 0, 0, 8, 'Partner Ledger'+' - '+data[0]['company_id'][1], self.format_title)
