odoo.define('account_dynamic_reports.DynamicTbMain', function(require) {
    ...
    function view_current_record(self){
        self.do_action({
    ...
    var DynamicFrMain = AbstractAction.extend({
        ...
        events: {
            ...
            'click #view_current_filters': 'view_current_filters',
            'click #open_settings': 'open_settings',
            'click #pdf': 'print_pdf',
            'click #pdf_portrait': 'print_pdf_portrait',
            'click #xlsx': 'print_xlsx',
            'click .view-source': 'view_gl',
            'click .collapser': 'update_display_detail',
        },
        init: function(view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
            this.wizard_model = 'ins.financial.report';
            this.account_report_id = code.context.account_report_id | null;
            this.session = session;
        },
        view_current_filters: function(event){
            event.preventDefault();
            view_current_record(this)
        },
        open_settings: function (event) {
            self = this
            self.do_action('account_dynamic_reports.action_ins_account_financial_report_tree');
        },
        start: function() {
            var self = this;
        ...
        print_pdf: function(e, landscape=1) {
            e.preventDefault();
            var self = this;
            self._rpc({
                model: 'ins.financial.report',
        ...
        print_pdf_portrait: function(e) {
            return this.print_pdf(e, 0)
        },
        print_xlsx: function() {
            var self = this;
            self._rpc({
        ...
        plot_data: function(initial_render = true) {
            ...
            self._rpc({
                ...
                _.each(self.account_data, function(k, v) {
                    ...
                    _.each(datas.form.month_list, function(m){
                        k[m] = self.formatWithSign(k[m], formatOptions, k[m] < 0 ? '-' : '');
                    })
                });
                if (initial_render) {
        ...
        view_gl: function(event) {
            ...
            domains.include_period_13 = self.filter_data.include_period_13
            var fr_wizard_id = 0;
            self._rpc({
                model: 'ins.general.ledger',
                method: 'create',
                args: [{date_from:this.filter_data.date_from,date_to:this.filter_data.date_to}]
            }).then(function(record) {
                fr_wizard_id = record;
                self._rpc({
                    model: 'ins.general.ledger',
        ...
        update_display_detail: function (event) {
            event.preventDefault();
            var self = this;
            var self_id = $(event.currentTarget).data('self-id');

            console.log(self_id);
            if ($('.a' + self_id).length > 0){
                var option
                if ($(event.currentTarget).hasClass('opened') && $('.a' + self_id).hasClass('show')) {
                    option = 'no_detail'
                    $(event.currentTarget).removeClass('opened').addClass('closed').removeClass('fa-caret-down').addClass('fa-caret-right');
                    self._rpc({
                        model: 'ins.account.financial.report',
                        method: 'update_display_detail',
                        args: [[self_id], option]
                        }).then(function(record){
                            console.log('1');
                    })
                } else if ($(event.currentTarget).hasClass('closed') && !$('.a' + self_id).hasClass('show')) {
                    option = 'detail_with_hierarchy'
                    $(event.currentTarget).removeClass('closed').addClass('opened').removeClass('fa-caret-right').addClass('fa-caret-down');
                    self._rpc({
                        model: 'ins.account.financial.report',
                        method: 'update_display_detail',
                        args: [[self_id], option]
    ...
    var DynamicGlMain = AbstractAction.extend({
        ...
        init: function(view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
            this.wizard_model = 'ins.general.ledger';
            this.session = session;
        },
        view_current_filters: function(event){
            event.preventDefault();
        ...
        plot_data: function(initial_render = true) {
            ...
            self._rpc({
                ...
                _.each(self.account_data, function(k, v) {
                    ...
                    k.credit = self.formatWithSign(k.credit, formatOptions, k.credit < 0 ? '-' : '');
                    k.balance = self.formatWithSign(k.balance, formatOptions, k.balance < 0 ? '-' : '');
                    k.amount_currency = self.formatWithSign(k.amount_currency, {currency_id: k.currency_id, noSymbol: true}, k.amount_currency < 0 ? '-' : '');
        ...
        update_with_filter: function(event) {
            ...
            for (var i = 0; i < options_list.length; i++) {
                ...
                if (options_list[i].id === 'include_period_13') {
                    output.include_period_13 = true;
                }
                if (options_list[i].id === 'include_exch_rate_entries') {
                    output.include_exch_rate_entries = true;
                }
            }
            if ($("#date_from").val()) {
                var dateObject = $("#date_from").datepicker("getDate");
    ...
    var DynamicAnlMain = AbstractAction.extend({
        template: 'DynamicAnlMain',
        events: {
            'click #filter_apply_button': 'update_with_filter',
            'click #view_current_filters': 'view_current_filters',
            'click #pdf': 'print_pdf',
        ...
        init: function(view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
            this.wizard_model = 'ins.analytic.report';
            this.session = session;
        },
        view_current_filters: function(event){
            event.preventDefault();
        ...
        print_pdf: function(e) {
            ...
            self._rpc({
                ...
                var action = {
                    'type': 'ir.actions.report',
                    'report_type': 'qweb-pdf',
                    'report_name': 'account_dynamic_reports.analytic_report',
                    'report_file': 'account_dynamic_reports.analytic_report',
                    'data': {
                        'js_data': data
                    },
                    'context': {
                        'active_model': 'ins.analytic.report',
                        'landscape': 1,
                        'from_js': true
                    },
                    'display_name': 'Analytic Report',
                };
                return self.do_action(action);
            });
        },
        ...
        plot_data: function(initial_render = true) {
            ...
            self._rpc({
                ...
                self.filter_data = datas[0]
                self.account_data = datas[1]
                self.final_balance = datas[2]
                var currency_id = false;
                // Rajeel | Analytic Report | 29/03/2023
                _.each(self.account_data, function(k1, v1) {
                    if (k1.balance != 0){
                        var formatOptions = {
                            currency_id: k1.lines[0].company_currency_id,
                            noSymbol: true,
                        };
                        k1.balance = self.formatWithSign(k1.balance, formatOptions, k1.balance < 0 ? '-' : '');
                    }
                    _.each(k1.lines, function (k,v) {
                        var formatOptions = {
                            currency_id: k.company_currency_id,
                            noSymbol: true,
                        };
                ...
                self.final_balance = self.formatWithSign(datas[2], formatOptions, datas[2] < 0 ? '-' : '');


                if (initial_render) {
                    self.$('.py-control-panel').html(QWeb.render('FilterSectionAnl', {
                        filter_data: datas[0],
                    }));
                    self.$el.find('#date_from').datepicker({
                        dateFormat: 'dd-mm-yy'
                ...
                self.$('.py-data-container-orig').html(QWeb.render('DataSectionAnl', {
                    account_data: datas[1],
        ...
        fetch_move_lines_by_page: function(event) {
            event.preventDefault();
            var self = this;
            var account_id = $(event.currentTarget).data('analytic-id');
            var offset = parseInt($(event.currentTarget).data('page-number')) - 1;
            var total_rows = parseInt($(event.currentTarget).data('count'));
            self.loader_disable_ui();
            self.gl_lines_by_page(offset, account_id).then(function(datas) {
        ...
        view_move_line: function(event) {
            ...
            var redirect_to_document = function(res_model, res_id, view_id) {
                ...
                if (String(res_id).includes(',')) {
                    delete(action['res_id'])
                    // delete(action['views'])
                    action['domain'] = [['id','in', res_id.split(',').map(Number)]]
            ...
            redirect_to_document('account.move.line', $(event.currentTarget).data('move-id'));
        },
        update_with_filter: function(event) {
            event.preventDefault();
            var self = this;
    ...
    var DynamicPaMain = AbstractAction.extend({
        ...
        init: function(view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
            this.wizard_model = 'ins.partner.ageing';
            this.session = session;
        },
        view_current_filters: function(event){
            event.preventDefault();
        ...
        plot_data: function(initial_render = true) {
            ...
            self._rpc({
                ...
                var partner_list = [];
                for (var z = 0; z < self.filter_data['partners_list'].length; z++) {
                    if (!partner_list.includes(self.filter_data['partners_list'][z][1])){
                        partner_list.push(self.filter_data['partners_list'][z][1])
                    }
                }
                partner_list.sort();
        		partner_list.push('Total')
                _.each(self.ageing_data, function(k, v) {
                    var currency_id = k.company_currency_id;
                    if (self.filter_data.currency_id) {
                        var currency_id = self.filter_data.currency_id
                    }
                    var formatOptions = {
                        currency_id: currency_id,
                        noSymbol: true,
                ...
                if (initial_render) {
                    ...
                    self.$el.find('.extra-multiple').select2({
                        ...
                        .val(['include_details', 'bal_not_zero']).trigger('change');
                }
    ...
    var DynamicPpMain = AbstractAction.extend({
        ...
        init: function(view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
            this.wizard_model = 'ins.partner.payment';
            this.session = session;
        },
        view_current_filters: function(event){
            event.preventDefault();
    ...
    var DynamicPiMain = AbstractAction.extend({
        ...
        init: function(view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
            this.wizard_model = 'ins.partner.invoice';
            this.session = session;
        },
        view_current_filters: function(event){
            event.preventDefault();
        ...
        plot_data: function(initial_render = true) {
            ...
            self._rpc({
                ...
                _.each(self.account_data, function(k, v) {
                    ...
                    var am_curr = k.amount_currency;
                    var am_comp_curr = k.amount_company_currency;
                    var am_unrec = k.amount_unreconciled;
                    var am_unrec_comp_curr = k.amount_unreconciled_company_currency;
                    k.amount_currency = self.formatWithSign(am_curr, formatOptions, am_curr < 0 ? '-' : '');
                    k.amount_company_currency = self.formatWithSign(am_comp_curr, formatOptionsCompanyCurr, am_comp_curr < 0 ? '-' : '');
                    k.amount_reconciled = self.formatWithSign(am_curr-am_unrec, formatOptions, (am_curr-am_unrec) < 0 ? '-' : '');
                    k.amount_reconciled_company_currency = self.formatWithSign(am_comp_curr-am_unrec_comp_curr, formatOptionsCompanyCurr, (am_comp_curr-am_unrec_comp_curr) < 0 ? '-' : '');
                    k.amount_unreconciled = self.formatWithSign(am_unrec, formatOptions, am_unrec < 0 ? '-' : '');
                    k.amount_unreconciled_company_currency = self.formatWithSign(am_unrec_comp_curr, formatOptionsCompanyCurr, am_unrec_comp_curr < 0 ? '-' : '');
    ...
    var DynamicPlMain = AbstractAction.extend({
        ...
        init: function(view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
            this.wizard_model = 'ins.partner.ledger';
            this.session = session;
        },
        view_current_filters: function(event){
            event.preventDefault();
            ...
            self._rpc({
                ...
                _.each(self.account_data, function(k, v) {
                    ...
                    _.each(k.lines, function(ks, vs) {
                        ...
                        ks.credit = self.formatWithSign(ks.credit, formatOptions, ks.credit < 0 ? '-' : '');
                        ks.balance = self.formatWithSign(ks.balance, formatOptions, ks.balance < 0 ? '-' : '');
                        ks.amount_currency = self.formatWithSign(ks.amount_currency, {currency_id: ks.currency_id, noSymbol: true}, ks.amount_currency < 0 ? '-' : '');
                        ks.ldate = field_utils.format.date(field_utils.parse.date(ks.ldate, {}, {
                            isUTC: true
                        }));
                    });
    ...
    var DynamicTbMain = AbstractAction.extend({
        ...
        init: function(view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
            this.wizard_model = 'ins.trial.balance';
            this.session = session;
        },
        view_current_filters: function(event){
            event.preventDefault();
        ...
        plot_data: function(initial_render = true) {
            ...
            self._rpc({
                ...
                self.$('.py-data-container').html(QWeb.render('DataSectionTb', {
                    account_data: self.account_data,
                    account_data_sorted_keys: Object.keys(self.account_data,).sort(),
    ...
    var DynamicUnfrMain = AbstractAction.extend({
        template: 'DynamicUnfrMain',
        events: {
            // 'click #filter_apply_button': 'update_with_filter',
            'click #view_current_filters': 'view_current_filters',
            'click #pdf': 'print_pdf',
            'click #xlsx': 'print_xlsx',
            'click .view-source': 'view_move_line',
            'click #adjustment_entry': 'open_adjustment_entry',
            // 'click .py-mline': 'fetch_move_lines',
            // 'click .py-mline-page': 'fetch_move_lines_by_page'
        },
        init: function(view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
            this.wizard_model = 'unrealized.forex.report';
            this.session = session;
        },
        view_current_filters: function(event){
            event.preventDefault();
        ...
        view_move_line: function(event) {
            ...
            var action = {
                ...
                res_id: $(event.currentTarget).data('move-id') ,
                target: 'current',
                context: context,
            };
            self.do_notify(_("Redirected"), "Window has been redirected");
        ...
        format_with_currency: function(values, formatOptions, formatOptions_cc){
            values.report_amount_currency = this.formatWithSign(values.report_amount_currency, formatOptions, values.report_amount_currency < 0 ? '-' : '');
            values.report_balance = this.formatWithSign(values.report_balance, formatOptions_cc, values.report_balance < 0 ? '-' : '');
            values.report_amount_currency_current = this.formatWithSign(values.report_amount_currency_current, formatOptions_cc, values.report_amount_currency_current < 0 ? '-' : '');
            values.report_adjustment = this.formatWithSign(values.report_adjustment, formatOptions_cc, values.report_adjustment < 0 ? '-' : '');
        },
        plot_data: function(initial_render = true) {
            var self = this;
            self.loader_disable_ui();
            ...
            self._rpc({
                ...
                _.each(self.Data, function (k, v) {
                    var formatOptions_cc = {currency_id: k.company_currency_id, noSymbol: true};
                    var formatOptions = {currency_id: k.currency_id, noSymbol: true};
                    self.format_with_currency(k, formatOptions, formatOptions_cc);
                    _.each(k['lines'], function (k2, v2) {
                        var formatOptions_cc = {currency_id: k2.company_currency_id, noSymbol: true};
                        var formatOptions = {currency_id: k2.currency_id, noSymbol: true};
                        self.format_with_currency(k2, formatOptions, formatOptions_cc);
                        _.each(k2['lines'], function (k3, v3) {
                            var formatOptions_cc = {currency_id: k3.company_currency_id, noSymbol: true};
                            var formatOptions = {currency_id: k3.currency_id, noSymbol: true};
                            self.format_with_currency(k3, formatOptions, formatOptions_cc);
                        })
                    })
                })
                if (initial_render) {
                    self.$('.py-control-panel').html(QWeb.render('FilterSectionUnfr', {
                        filter_data: datas[0],
                    }));
                }
                self.$('.py-data-container-orig').html(QWeb.render('DataSectionUnfr', {
                        Data: datas[1]
                    }));
                self.loader_enable_ui();
            });
        },
        open_adjustment_entry: function (){
            self=this
            this._rpc({
                model: 'unrealized.forex.revaluation',
                method: 'create',
                args: [{
                    report_wizard_id: this.wizard_id
                }]
            }).then(function(record) {
                var context = {report_wizard_id: this.wizard_id}
                var action = {
                    name: "Create Adjustment Entry",
                    type: 'ir.actions.act_window',
                    view_type: 'form',
                    view_mode: 'form',
                    res_model: 'unrealized.forex.revaluation',
                    views: [
                        [false, 'form']
                    ],
                    res_id: record,
    ...
    var DynamicAssetMain = AbstractAction.extend({
        template: 'DynamicAssetMain',
        events: {
            // 'click #filter_apply_button': 'update_with_filter',
            'click #view_current_filters': 'view_current_filters',
            // 'click #pdf': 'print_pdf',
            'click #xlsx': 'print_xlsx',
            'click .view-source': 'view_asset',
            // 'click .py-mline': 'fetch_move_lines',
            // 'click .py-mline-page': 'fetch_move_lines_by_page'
        },
        init: function(view, code) {
            this._super(view, code);
            this.wizard_id = code.context.wizard_id | null;
            this.wizard_model = 'account.asset.report';
            this.session = session;
        },
        view_current_filters: function(event){
            event.preventDefault();
        ...
        view_asset: function(event) {
            ...
            var action = {
                ...
                res_id: $(event.currentTarget).data('asset-id') ,
                target: 'current',
                context: context,
            };
            self.do_notify(_("Redirected"), "Window has been redirected");
        ...
        format_with_currency: function(values, formatOptions){
            values.paid_amount = this.formatWithSign(values.paid_amount, formatOptions, values.paid_amount < 0 ? '-' : '');
            values.balance_b_f = this.formatWithSign(values.balance_b_f, formatOptions, values.balance_b_f < 0 ? '-' : '');
            values.addition = this.formatWithSign(values.addition, formatOptions, values.addition < 0 ? '-' : '');
            values.cost_total = this.formatWithSign(values.cost_total, formatOptions, values.cost_total < 0 ? '-' : '');
            values.charged_out_total = this.formatWithSign(values.charged_out_total, formatOptions, values.charged_out_total < 0 ? '-' : '');
            values.balance_c_f = this.formatWithSign(values.balance_c_f, formatOptions, values.balance_c_f < 0 ? '-' : '');
            values.monthly_amortization = this.formatWithSign(values.monthly_amortization, formatOptions, values.monthly_amortization < 0 ? '-' : '');
        },
        plot_data: function(initial_render = true) {
            var self = this;
            self.loader_disable_ui();
            ...
            self._rpc({
                ...
                _.each(self.Data, function (k, v) {
                    var formatOptions = {currency_id: k.currency_id, noSymbol: true};
                    self.format_with_currency(k, formatOptions);
                })
                if (initial_render) {
                    self.$('.py-control-panel').html(QWeb.render('FilterSectionAsset', {
                        filter_data: datas[0],
                    }));
                }
                self.$('.py-data-container-orig').html(QWeb.render('DataSectionAsset', {
                        Data: datas[1]
                    }));
                self.loader_enable_ui();
            });
    ...
    core.action_registry.add('dynamic.anl', DynamicAnlMain);
    core.action_registry.add('dynamic.pa', DynamicPaMain);
    core.action_registry.add('dynamic.pl', DynamicPlMain);
    core.action_registry.add('dynamic.pp', DynamicPpMain);
    core.action_registry.add('dynamic.pi', DynamicPiMain);
    core.action_registry.add('dynamic.tb', DynamicTbMain);
    core.action_registry.add('dynamic.ufrx', DynamicUnfrMain);
    core.action_registry.add('dynamic.asset', DynamicAssetMain);
});