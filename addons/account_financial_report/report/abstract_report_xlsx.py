...
class AbstractReportXslx(models.AbstractModel):
    ...
    def _define_formats(self, workbook):
        ...
        self.format_sst_report_amount = workbook.add_format()
        self.format_sst_report_amount.set_num_format('#,##0.' + '0' * 2)
    ...
    def write_line(self, line_object):
        ...
        for col_pos, column in self.columns.items():
            ...
            if cell_type == 'many2one':
                ...
                line_object.currency_id = self.env.user.company_id.currency_id
                ...
                original_date_str = value
                if original_date_str:
                    date_obj = dt.strptime(original_date_str, '%Y-%m-%d')  # Parse with original format
                    desired_format = "%d-%m-%Y"
                    formatted_date_str = date_obj.strftime(desired_format)
                    self.sheet.write_string(
                        self.row_pos, col_pos, formatted_date_str
                    )
                else:
                    self.sheet.write_string(
            ...
            elif cell_type == 'sst_report_amount':
                self.sheet.write_number(self.row_pos, col_pos, float(value), self.format_sst_report_amount)
