    ...
    def action_create_invoice(self):
        if self.booking_no:
            ...
            if create_invoice:
                ...
                elif booking.customer_name.user_id:
                    salesperson_id = booking.customer_name.user_id.id
                inv_currency_id = self.env.user.company_id.currency_id.id
                inv_currency_name = self.env.user.company_id.currency_id.name
                if booking.customer_name.property_product_pricelist:
                    if booking.customer_name.property_product_pricelist.currency_id:
                        inv_currency_id = booking.customer_name.property_product_pricelist.currency_id.id
                        inv_currency_name = booking.customer_name.property_product_pricelist.currency_id.name
                sale_currency_rate = 1.000000
                apply_manual_currency_exchange = False
                if self.cost_profit_ids:
                    if self.cost_profit_ids[0].sale_currency_rate > 0 and inv_currency_name != self.env.user.company_id.currency_id.name:
                        sale_currency_rate = self.cost_profit_ids[0].sale_currency_rate
                ...
                inv_val = {
                    ...
                    'exchange_rate_inverse': 4.5,
                    'apply_manual_currency_exchange': apply_manual_currency_exchange,
                }
                invoice = inv_obj.with_context(create_from_job=True).create(inv_val)
                for booking_line in self.cost_profit_ids:
                    if booking_line.add_to_invoice:
                        ...
                        if line_item.profit_currency.id != inv_currency_id:
                            freight_foreign_price = sale_unit_price_converted
                            freight_currency_rate = 1 / invoice.exchange_rate_inverse
                            price_unit = freight_foreign_price * freight_currency_rate
                            cp_up = price_unit * invoice.exchange_rate_inverse
                        else:
                            freight_foreign_price = line_item.list_price
                            freight_currency_rate = 1
                            price_unit = line_item.list_price
                            cp_up = line_item.list_price

                        line_item.write({
                            'list_price': cp_up,
                            'profit_currency_rate': freight_currency_rate,
                            'profit_currency': inv_currency_id
                        })
                        ...
                        if account_id:
                            if price_unit > 0:
                                inv_line = inv_line_obj.with_context(create_from_job=True).create({
                                    ...
                                    'freight_foreign_price': freight_foreign_price or 0.00,
                                    'freight_currency_rate': float_round(freight_currency_rate, 6, rounding_method='HALF-UP') or 1.000000,
                                    'uom_id': uom_id or False,
                                    'price_unit': float_round(price_unit, 6, rounding_method='HALF-UP') or 0.00,
