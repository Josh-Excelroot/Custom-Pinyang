...
class AccountPaymentMoveLine(models.Model):
    ...
    name = fields.Char(string='Label',index=True)
...
class PaymentMoveLine(models.Model):
    ...
    ref = fields.Char(related='move_line_id.ref',readonly=True, index=True)
    name = fields.Char(related='move_line_id.move_id.name', readonly=True, index=True)
    ...
    move_line_id = fields.Many2one('account.move.line', required=True, ondelete='cascade',index=True)
    ...
    allocation_currency_id = fields.Many2one(related='payment_id.currency_id', )
