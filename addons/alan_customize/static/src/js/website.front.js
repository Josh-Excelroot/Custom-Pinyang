odoo.define('alan_customize.front_js',function(require){
    ...
    function initialize_owl(el,auto_slider,timer){
        el.owlCarousel({
            items: 4,
            margin: 25,
            navigation: true,
    ...
    function initialize_owl_1(el,auto_slider,timer){
        el.owlCarousel({
            items: 4,
            margin: 25,
            navigation: true,
                ...
                $.get("/shop/get_product_snippet_content",{
                    ...
                    if(data){
                        ...
                        if(self.$target.attr('data-snippet_layout') === 'slider' && self.$target.find('> .prod_slider').length === 1){
                            initialize_owl_1(self.$target.find("> .prod_slider .tqt-pro-slide"), auto_slider, slider_timer);
                        }
                        else if(self.$target.attr('data-snippet_layout') === 'fw_slider' && self.$target.find('> .fw_prod_slider').length === 1){
                            initialize_owl(self.$target.find("> .fw_prod_slider .tqt-pro-slide"), auto_slider, slider_timer);
                        }
...
//            kashif condition code to have oe_sruct class : 8mar 24
            else{
//            self.$target.addClass('oe_structure')
            }
        }
    });
