...
class WebsiteSale(WebsiteSale):
    ...
    def get_products_content(self, **post):
        ...
        if post.get('tab_id'):
            tab_data = request.env['multitab.configure'].browse(
                int(post.get('tab_id')))
            return request.render("alan_customize.product_slider_content", {'product_collection': tab_data,'inner': tab_data.inner_html, 'compute_currency': compute_currency, 'limit': post.get('limit') or 0, 'full_width': post.get('full_width')})
        return ''
    ...
    def get_product_brand_slider(self, **post):
        ...
        if post.get('brand-count'):
            brand_ids = request.env['product.brand.website'].search(
                [('visible_slider', '=', True)] + request.website.website_domain())
            if brand_ids:
    ...
    def get_product_latest_p(self, tab_id=0, **post):
        ...
        if tab_id:
            ...
            if tab:
                value.update(
                    {'tab_obj': tab, 'compute_currency': compute_currency})
    ...
    def get_multi_tab_content(self, **post):
        ...
        if post.get('collection_id'):
            ...
            value.update({'inner': collection_data.inner_html})
    ...
    def get_brand_multi_tab_content(self, **post):
        value = {}
        if post.get('label'):
            value['header'] = post.get('label')
        if post.get('collection_id') and post.get('collection_id') != 'false':
            ...
            value.update({'obj': collection_data})
            return {'auto_slider_value': auto_slider_value,
                    'slider_timing': slider_timing,
                    'item_count': item_count,
                    'slider': request.env.ref("alan_customize.s_brand_collection_configure").render(value)}
        return ""
    ...
    def product_slider_collection(self, **post):
        ...
        if post.get('collection_id'):
            collection = request.env['multitab.configure'].browse(
                int(post.get('collection_id')))
            if collection:
                value.update({'product_collection': collection,
    ...
    def get_product_img_gallery(self, **post):
        ...
        if 'product_id' in post and post.get('product_id'):
            p_var_rec = request.env['product.product'].search(
                [('id', '=', int(post.get('product_id')))])
            value['variant_img'] = p_var_rec if len(
                p_var_rec.product_tmpl_id.product_variant_ids) > 1 else False
