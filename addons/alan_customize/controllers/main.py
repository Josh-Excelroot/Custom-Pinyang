...
class WebsiteSale(WebsiteSale):
    ...
    def product_brands(self, **post):
        cr, context, pool = (request.cr,
                             request.context,
                             request.registry)
        b_obj = request.env['product.brand.website']
        domain = []
        if post.get('search'):
    ...
    def _get_search_domain_ext(self, search, category, attrib_values,
        ...
        if brand_values:
            domain += [('product_brand_website_id', 'in', brand_values)]
    ...
    @http.route([
        '''/shop''',
        '''/shop/page/<int:page>''',
        '''/shop/brand/<model("product.brand.website", "[('website_id', 'in', (False, current_website_id))]"):brand>''',
        '''/shop/brand/<model("product.brand.website", "[('website_id', 'in', (False, current_website_id))]"):brand>/page/<int:page>''',
    ...
    def shop(self, page=0, category=None, brand=None, search='', ppg=False, **post):
        ...
        if brand:
            brand = request.env['product.brand.website'].search(
        ...
        if brand:
            domain += [('product_brand_website_id', '=', brand.id)]

        if post.get('brand'):
            product_designer_obj = request.env['product.brand.website']
            brand_ids = product_designer_obj.search(
                [('id', '=', int(post.get('brand')))] + request.website.website_domain())
            if brand_ids:
                domain += [('product_brand_website_id', 'in', brand_ids.ids)]
        ...
        if brand:
            brand = request.env['product.brand.website'].browse(int(brand))
        ...
        brands = request.env['product.brand.website'].search([])
