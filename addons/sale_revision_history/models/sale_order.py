...
class SaleOrder(models.Model):
    ...
    current_revision_id = fields.Many2one(
        'sale.order', 'Current revision', readonly=True, copy=True)
    old_revision_ids = fields.One2many(
        'sale.order', 'current_revision_id', 'Old revisions', readonly=True, context={'active_test': False})
    ...
    def copy(self, defaults=None):
        ...
        if self.old_revision_ids and self.old_revision_ids.sorted(key=lambda x: x.id)[0].name != self.unrevisioned_name:
            self.unrevisioned_name = self.old_revision_ids.sorted(key=lambda x: x.id)[0].name
        if self.env.context.get('sale_revision_history'):
            ...
            defaults.update({'name': prev_name, 'revision_number': revno, 'active': False, 'state': 'cancel',
