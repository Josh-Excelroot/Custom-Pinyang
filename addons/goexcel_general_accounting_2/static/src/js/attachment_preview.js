...
odoo.define('goexcel_general_accounting_2.invoice_previews', function (require) {
    'use strict';
...
//            this.__parentedParent.__parentedParent.$el.removeClass('attachment_preview');
        ...
        console.log("SAJJAD ATTACH",atachments)
        if (atachments){
...
//            self.attachments = result;
            ...
            if(event.data.attrs.id === "preview_report_invoice_view"){
...
//kashif 2jan25 : addcalss js issue fix
        hide: function () {
        console.log("HIDE");
        if(this.$el){
        ...
        if (this.attachments){
            console.log("SAJJAD",this.attachments)
...
////            }
          ...
          var self = this;
           var def = this._super();
           if(this.state.model === 'hr.payslip' || this.state.model === 'account.invoice'){
