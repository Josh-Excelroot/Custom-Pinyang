...
class AccountAssetCategory(models.Model):
    ...
    account_asset_id = fields.Many2one('account.account', string='Asset Account', required=True, domain=[(
        'internal_type', '=', 'other'), ('deprecated', '=', False)], help="Account used to record the purchase of the asset at its original price.")
    account_depreciation_id = fields.Many2one('account.account', string='Depreciation Entries: Asset Account', required=True, domain=[(
        'internal_type', '=', 'other'), ('deprecated', '=', False)], help="Account used in the depreciation entries, to decrease the asset value.")
    account_depreciation_expense_id = fields.Many2one('account.account', string='Depreciation Entries: Expense Account', required=True, domain=[('internal_type', '=', 'other'), (
        'deprecated', '=', False)], oldname='account_income_recognition_id', help="Account used in the periodical entries, to record a part of the asset as expense.")
...
class AccountAssetAsset(models.Model):
    ...
    code = fields.Char(string='Reference', size=32, readonly=True, states={
                       'draft': [('readonly', False)]})
    value = fields.Float(string='Gross Value', required=True, readonly=True, digits=0, states={
                         'draft': [('readonly', False)]}, oldname='purchase_value')
    ...
    date = fields.Date(string='Date', required=True, readonly=True, states={'draft': [
                       ('readonly', False)]}, default=fields.Date.context_today, oldname="purchase_date")
    ...
    method = fields.Selection([('linear', 'Linear'), ('degressive', 'Degressive')], string='Computation Method', required=True, readonly=True, states={'draft': [('readonly', False)]}, default='linear',
                              help="Choose the method to use to compute the amount of depreciation lines.\n  * Linear: Calculated on basis of: Gross Value / Number of Depreciations\n"
                              "  * Degressive: Calculated on basis of: Residual Value * Degressive Factor")
    method_number = fields.Integer(string='Number of Depreciations', readonly=True, states={'draft': [(
        'readonly', False)]}, default=5, help="The number of depreciations needed to depreciate your asset")
    ...
    method_end = fields.Date(string='Ending Date', readonly=True, states={
                             'draft': [('readonly', False)]})
    method_progress_factor = fields.Float(
        string='Degressive Factor', readonly=True, default=0.3, states={'draft': [('readonly', False)]})
    value_residual = fields.Float(
        compute='_amount_residual', method=True, digits=0, string='Residual Value')
    ...
    prorata = fields.Boolean(string='Prorata Temporis', readonly=True, states={'draft': [('readonly', False)]},
                             help='Indicates that the first depreciation entry for this asset have to be done from the exact date (Date) instead of the first day of the Month/Year.')
    depreciation_line_ids = fields.One2many('account.asset.depreciation.line', 'asset_id', string='Depreciation Lines', readonly=True, states={
    ...
    invoice_id = fields.Many2one('account.invoice', string='Invoice', states={'draft': [(
        'readonly', False)]}, copy=False, domain=[('state', 'not in', ['draft', 'cancel'])])
    ...
    def _compute_board_undone_dotation_nb(self, depreciation_date, total_days):
        ...
        if self.method_time == 'end':
            end_date = strptime(self.method_end, DF).date()
        ...
        if self.value_residual != 0.0:
            ...
            if self.prorata:
                ...
                if posted_depreciation_line_ids and posted_depreciation_line_ids[-1].depreciation_date:
                    last_depreciation_date = strptime(
                ...
                if self.method_period >= 12:
                    if self.company_id.fiscalyear_last_month:
                        ...
                        asset_date = strptime(
                            self.date.year + '-01-01', DF).date()
                else:
                    #asset_date = strptime(self.date[:7] + '-01', DF).date()
    ...
    def _get_disposal_moves(self):
        ...
        for asset in self:
            ...
            if unposted_depreciation_line_ids:
                ...
                sequence = len(asset.depreciation_line_ids) - \
                    len(unposted_depreciation_line_ids) + 1
                ...
                if changes:
                    asset.message_post(subject=_(
                        'Asset sold or disposed. Accounting entry awaiting for validation.'), tracking_value_ids=tracking_value_ids)
    ...
    def onchange_invoice_id(self):
        self.value = sum(self.invoice_id.invoice_line_ids.filtered(
            lambda l: l.asset_category_id).mapped('price_total'))
    ...
    def create(self, vals_list):
        records = super(AccountAssetAsset, self.with_context(
...
class AccountAssetDepreciationLine(models.Model):
    ...
    remaining_value = fields.Float(
        string='Next Period Depreciation', digits=0, required=True)
    depreciated_value = fields.Float(
        string='Cumulative Depreciation', required=True)
    depreciation_date = fields.Date('Depreciation Date', index=True)
    move_id = fields.Many2one('account.move', string='Depreciation Entry')
    move_check = fields.Boolean(
        compute='_get_move_check', string='Linked', track_visibility='always', store=True)
    ...
    def create_move(self, post_move=True):
        ...
        if post_move and created_moves:
            created_moves.filtered(lambda m: any(
                m.asset_depreciation_ids.mapped('asset_id.category_id.open_asset'))).post()
    ...
    def log_message_when_posted(self):
        ...
        for line in self:
            if line.move_id and line.move_id.state == 'draft':
                ...
                msg = _format_message(
                    _('Depreciation line posted.'), msg_values)
