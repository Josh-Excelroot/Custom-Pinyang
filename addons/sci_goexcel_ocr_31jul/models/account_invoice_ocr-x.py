        ...
        for line in text_file:
            if len(line) > 3 and not check_date:
                ...
                if get_date_next_line:
                    string = line.split()
                    for i in string:
                        if check_date and "-" in i and "-" in date_pattern_ori:
    ...
    def get_due_date(self, partner_template, text_file_dic, date):
        ...
        for line in text_file:
            if len(line) > 3 and not check_due_date:
                ...
                month_in_line = False
                year_in_line = False
                date_text_found = False
                month = "01"
                if partner_template:
                    if partner_template.date_format:
                        date_pattern = partner_template.date_format.replace(" ", "")
                for keyword in due_date_keywords.mapping_line_ids:
                    if keyword.keyword in line or (partner_template.name == 'MEDITERRANEAN SHIPPING COMPANY (M) SDN BHD' and date_text_found):
                        if not date_text_found and partner_template.name == 'MEDITERRANEAN SHIPPING COMPANY (M) SDN BHD':
                            date_text_found = True
                            continue
                        if partner_template.name == 'MEDITERRANEAN SHIPPING COMPANY (M) SDN BHD':
        ...
        if not payment_term:
            if date and due_date:
                ...
                payment_term_ids = self.env['account.payment.term.line'].search([
                    ...
                    ('days', '=', delta.days),
                    ('payment_id.company_id', '=', self.env.user.company_id.id)
                ], limit=1)
                    ...
                    if partner_template.complex_line:
                        ...
                        if 0 < len(price_list) == x:
                            ...
                            if "TOTAL" in line_item_pattern:
                                ...
                                for temp_name in temp_names:
                                    if not temp_name.isdecimal() and len(temp_name) > 2:
                                        if '.' not in temp_name:
                                            # Get Product Name
                                            if not product_name:
                                                if len(temp_name) > 1:
