...
class print_customer_statement_xlsx(models.AbstractModel):
    ...
    def get_lines_open(self, partner):
        ...
        for currency, lines in data.items():
            ...
            for line in lines:
                ...
                if not line.get('date'):
                    ...
                    if show_term:
                        sheet.write_string(start_row, 4, line.get('payment_term'), self.line_header)
            ...
            # DATA SUB-TOTAL
            start_row += 1
            sheet.merge_range(start_row, 0, start_row, 2 + show_term, 'Sub-Total', self.format_header)
            sheet.write_string(start_row, 3 + show_term, self.set_amount(total_debit), self.line_header)
            sheet.write_string(start_row, 4 + show_term, self.set_amount(total_credit), self.line_header)
            sheet.write_string(start_row, 5 + show_term, self.set_amount(total), self.line_header)
            ...
            if partner.soa_type == 'all':
                ...
                aging = self.set_ageing_all(partner)[currency]

            if partner.aging_group == 'by_month':
                headings = "Current Month,1 Month,2 Months,3 Months,4 Months,5 Months & Above,Total".split(',')
                del aging[0]['not_due']
            else:
            ...
            for idx, heading in enumerate(headings):
                sheet.write_string(start_row, idx, heading, self.format_header)
            start_row += 1
            for idx, amount in enumerate(aging[0].values()):
