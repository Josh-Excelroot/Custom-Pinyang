...
class ShippingInstruction(models.Model):
    ...
    booking_ref = fields.Many2one(
        "freight.booking",
        string="Booking Job Ref",
        track_visibility="onchange",
        copy=False,
        index=True,
    )
    bl_ref = fields.Many2one(
        "freight.bol",
        string="BL Ref",
        track_visibility="onchange",
        copy=False,
    ...
    carrier_booking_ref = fields.Char(
        string="Carrier Booking Ref.", track_visibility="onchange", copy=False
    )
    customer_ref = fields.Char(
        string="Customer Ref.", track_visibility="onchange", copy=False
    )
    ...
    bill_of_lading_type = fields.Selection(
        [("original", "Original"), ("telex", "Telex"), ("seaway", "Seaway")],
        string="Bill of Lading Type",
        track_visibility="onchange",
    )
    bol_status = fields.Selection(
        [("01", "Draft"), ("02", "Original"), ("03", "Surrender")],
        string="BOL Status",
        default="01",
    )
    no_of_original_bl = fields.Selection(
        [("0", "0"), ("1", "1"), ("3", "3")], string="No Of original B/L", default="0"
    )
    note = fields.Text(string="Remarks", track_visibility="onchange")

    port_of_loading_input = fields.Many2one("freight.ports", string="Port of Loading")
    port_of_discharge_input = fields.Many2one(
        "freight.ports", string="Port of Discharge"
    )
    place_of_delivery = fields.Char(
        string="Place of Delivery", track_visibility="onchange"
    )
    place_of_receipt = fields.Char(
        string="Place of Receipt", track_visibility="onchange"
    )
    shipping_agent = fields.Char(
        string="Oversea Shipping Agent", track_visibility="onchange"
    )

    fcl_line_ids = fields.One2many(
        "freight.website.si.fcl",
        "fcl_line",
        string="FCL Line",
        copy=True,
        auto_join=True,
    ...
    lcl_line_ids = fields.One2many(
        "freight.website.si.lcl",
        "lcl_line",
        string="LCL Line",
        copy=True,
        auto_join=True,
    ...
    si_file_name = fields.Char(
        string="SI File name", track_visibility="onchange", copy=False
    )
    si_attachment = fields.Binary(
        string="SI Attachment", track_visibility="onchange", copy=False
    )
    ...
    processor = fields.Many2one(
        "res.users", string="Processor", track_visibility="onchange"
    )
    ...
    def get_data(self, data):
        converted_line_id = int(data[0])
        si_cargo_type = data[1]

        if si_cargo_type == "fcl":
            line_data = (
                self.env["freight.website.si.fcl"]
                .sudo()
                .search([("id", "=", converted_line_id)])[0]
            )

        if si_cargo_type == "lcl":
            line_data = (
                self.env["freight.website.si.lcl"]
                .sudo()
                .search([("id", "=", converted_line_id)])[0]
            )
        ...
        transformed_data = {
            "id": line_data.id,
            "container_product_name": line_data.container_product_name,
            "fcl_container_qty": line_data.fcl_container_qty,
            "seal_no": line_data.seal_no,
            "packages_no": line_data.packages_no,
            "exp_gross_weight": line_data.exp_gross_weight,
            "remark": line_data.remark,
            "exp_vol": line_data.exp_vol,
        }
        return transformed_data

    def open_edit(self, cr, uid, ids, context=None):
        return {
            "type": "ir.actions.act_window",
            "view_type": "form",
            "view_mode": "form",
            "res_model": "freight.website.si",  # name of respective model,
            "view_id": "sci_goexcel_freight.",
            "target": "current",
            "context": context,
        }
                        ...
                        op_line = operation_line_obj.create(
                            {
                                ...
                                "container_product_id": line.container_product_id.id
                                or False,
                                "container_commodity_id": line.container_commodity_id.id
                                or False,
                                "container_product_name": line.container_product_name
                                or "",
                        ...
                        op_line = operation_line_obj.create(
                            {
                                ...
                                "container_product_id": line.container_product_id.id
                                or False,
                                "container_commodity_id": line.container_product_id.id
                                or False,
                                "container_product_name": line.container_product_name
                                or "",
                                "container_no": line.container_no or "",
    ...
    def action_send_si(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference(
                "sci_goexcel_freight", "email_template_si"
            )[1]
        except ValueError:
            template_id = False
        try:
    ...
    def action_send_si_xlsx(self):
        return self.env.ref("sci_goexcel_freight.action_si_report_xlsx").report_action(
            self
        )
...
class FCLLine(models.Model):
    ...
    container_product_name = fields.Text(
        string="Description", track_visibility="onchange"
    )
    packages_no = fields.Integer(string="No. of Packages", track_visibility="onchange")
    packages_no_uom = fields.Many2one(
        "uom.uom", string="UoM", track_visibility="onchange"
    )
    exp_net_weight = fields.Float(
    ...
    exp_vol = fields.Float(
        string="Measurement (M3)",
        digits=(12, 4),
    ...
    container_commodity_id = fields.Many2one(
        "product.product", string="Commodity", track_visibility="onchange"
    )
    fcl_container_qty = fields.Float(
    ...
    def create(self, vals):
        ...
        if vals.get("container_product_id"):
            content = (
                content
                + "  \u2022 Container: "
                + str(vals.get("container_product_id"))
                + "<br/>"
            )
        if vals.get("container_product_name"):
            content = (
                content
                + "  \u2022 Description: "
                + str(vals.get("container_product_name"))
                + "<br/>"
            )
        if vals.get("container_no"):
            content = (
                content
                + "  \u2022 Container No: "
                + str(vals.get("container_no"))
                + "<br/>"
            )
        if vals.get("seal_no"):
            content = (
                content + "  \u2022 Seal no: " + str(vals.get("seal_no")) + "<br/>"
            )
        if vals.get("fcl_container_qty"):
            content = (
                content
                + "  \u2022 Qty: "
                + str(vals.get("fcl_container_qty"))
                + "<br/>"
            )
        if vals.get("packages_no"):
            content = (
                content + "  \u2022 Pckg No: " + str(vals.get("packages_no")) + "<br/>"
            )
        if vals.get("packages_no_uom"):
            content = (
                content
                + "  \u2022 Pckg No UoM: "
                + str(vals.get("packages_no_uom"))
                + "<br/>"
            )
...
class LCLLine(models.Model):
    ...
    exp_net_weight = fields.Float(
        string="Net Weight(KG)",
        help="Expected Net Weight in kg.",
        track_visibility="onchange",
    )
    exp_vol = fields.Float(
        string="Measurement (M3)",
...
class Website(models.Model):
    ...
    def get_port(self):
        port_of_loadings = self.env["freight.ports"].search([])
