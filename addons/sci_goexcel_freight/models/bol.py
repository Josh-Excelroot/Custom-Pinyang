...
class BillOfLading(models.Model):
    ...
    feeder_voyage = fields.Char(string='Feeder Voyage No', track_visibility='onchange')
    ...
    def action_copy_from_booking(self):
        ...
        for line in booking.cost_profit_ids:
            is_created = False
            for bol_cost in self.cost_profit_ids:
                if line.product_id.id == bol_cost.product_id.id:
                    is_created = True
            if not is_created:
    ...
    def action_create_si(self):
        ...
        if self.port_of_loading_input:
            pol = self.env["freight.ports"].search([('name', '=', self.port_of_loading_input)], limit=1)
            #booking = self.env['freight.booking'].search([('booking_no', '=', self.booking_no), ])
            if pol:
                si_val['port_of_loading_input'] = pol.id
        if self.port_of_discharge_input:
            pod = self.env["freight.ports"].search([('name', '=', self.port_of_discharge_input)], limit=1)
            if pod:
                si_val['port_of_discharge_input'] = pod.id
...
class CostProfit(models.Model):
    ...
    def create(self, vals):
        ...
        if content and len(content) > 0:
            res.bol_id.message_post(body=content)
    ...
    def write(self, vals):
        ...
        if content and len(content)>0:
               self.bol_id.message_post(body=content)
