...
class BookingReport(models.Model):
    ...
    profit_qty = fields.Float(string='Qty', digits=(12, 2), readonly=True)
    ...
    invoice_paid_status = fields.Selection([('01', 'New'),
                                            ('02', 'Partially Paid'),
                                            ('03', 'Fully Paid')],
                                           string="Invoice Paid Status", readonly=True)
    ...
    def _query(self, with_clause='', fields={}, groupby='', from_clause=''):
        ...
        select_ = """
            ...
            b.invoice_paid_status as invoice_paid_status,
            ...
            c.sale_total as sale_total,
            c.cost_total as cost_total,
            c.profit_total as profit_total,
        ...
        for field in fields.values():
            ...
            # Yulia 14022025 must add inner select at freight_operation_line
        from_ = """
                freight_cost_profit c
                      left outer join freight_booking b on (c.booking_id=b.id)
                      left outer join (
                        SELECT 
                            operation_id, 
                            MAX(container_product_id) AS container_product_id, 
                            SUM(packages_no) AS packages_no,
                            MAX(packages_no_uom) AS packages_no_uom,
                            SUM(exp_net_weight) AS exp_net_weight,
                            SUM(exp_gross_weight) AS exp_gross_weight,
                            SUM(exp_vol) AS exp_vol
                        FROM freight_operations_line 
                        GROUP BY operation_id
                    ) f ON b.id = f.operation_id
                      left outer join res_partner partner on b.customer_name = partner.id
                        left outer join product_product p on (c.product_id=p.id)
                            left outer join product_template t on (p.product_tmpl_id=t.id)
                    left outer join uom_uom u on (u.id=c.uom_id)
                    left outer join uom_uom u2 on (u2.id=t.uom_id)
                %s
        """ % from_clause
