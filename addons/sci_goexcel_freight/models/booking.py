...
class FreightBooking(models.Model):
    ...
    invoice_ids = fields.One2many('account.invoice', 'freight_booking', string='Invoice', copy=False)
    ...
    unstuff_at = fields.Char(string='Warehouse', track_visibility='onchange')

    shipment_type = fields.Selection([('house', 'House'), ('direct', 'Direct')], string='Shipment Type',
                                     track_visibility='onchange', default='direct')
    ...
    port_of_loading_eta = fields.Date(string='Loading ETA', track_visibility='onchange', copy=False)
    ...
    scn_code = fields.Char(string='SCN Code', track_visibility='onchange', help='Ship Call Number')
    ...
    #                         'port_of_loading_eta': self.port_of_loading_eta,
    #                         'unstuff_date': self.port_of_discharge_eta,
    ...
    def action_create_bl(self):
        #print('>>>>>>>> booking action_create_bl')
        if self.service_type == 'ocean':
            # _logger.warning('export and ocean')
            # _logger.warning('si id=' + str(si.id))
            if self.cargo_type == 'fcl':
                ...
                if not self.lcl_consolidation:
                    #print('>>>>>>>> booking action_create_bl fcl not lcl_consolidation')
                ...
                for line in container_line:
                    ...
                    if self.direction != 'import' and not self.lcl_consolidation:
                        #print('>>>>>>>> booking action_create_bl lcl not lcl_consolidation')
                        bol_val['customer_name'] = self.customer_name.id
                        bol_val['contact_name'] = self.contact_name.id
                        bol_val['shipper'] = self.shipper_address_input
                        bol_val['consignee'] = self.consignee_address_input
    ...
    def action_create_si(self):
        ...
        si_val = {
            ...
            'voyage_no': self.voyage_no,
            'vessel': self.vessel_name.name,
            'port_of_loading_input': self.port_of_loading.id,
            'port_of_discharge_input': self.port_of_discharge.id,
            'place_of_delivery': self.place_of_delivery,
        ...
        if self.cargo_type == 'fcl':
            ...
            for line in container_line:
                if line.container_product_id or line.container_no or line.container_product_name:
    ...
    def onchange_state(self):
        ...
        if self.vessel_name:
            #print('onchange_state vessel_name=', self.vessel_name.name, ' self.id=', self._origin.id)
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self._origin.id),]):
                si.write({'vessel': self.vessel_name.name})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self._origin.id),]):
                bl.write({'vessel': self.vessel_name.name})


    @api.onchange('voyage_no')
    def onchange_voyage_no(self):
        #TODO
        if self.voyage_no:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self._origin.id),]):
                si.write({'voyage_no': self.voyage_no})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self._origin.id),]):
                bl.write({'voyage_no': self.voyage_no})


    @api.onchange('feeder_vessel_name')
    def onchange_feeder_vessel_name(self):
        if self.feeder_vessel_name:
            # for si in self.env['freight.website.si'].search([('booking_ref', '=', self.id),]):
            #     si.write({'voyage_no': self.feeder_vessel_name})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self._origin.id),]):
                bl.write({'pre_carriage_by': self.feeder_vessel_name})


    @api.onchange('feeder_voyage_no')
    def onchange_feeder_voyage_no(self):
        if self.feeder_voyage_no:
            # for si in self.env['freight.website.si'].search([('booking_ref', '=', self.id),]):
            #     si.write({'voyage_no': self.feeder_vessel_name})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self._origin.id),]):
                bl.write({'feeder_voyage': self.feeder_voyage_no})

    @api.onchange('port_of_loading_eta')
    def onchange_port_of_loading_eta(self):
        if self.port_of_loading_eta:
            # for si in self.env['freight.website.si'].search([('booking_ref', '=', self._origin.id),]):
            #     si.write({'date_laden_on_board': self.port_of_loading_eta})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self._origin.id),]):
                bl.write({'date_laden_on_board': self.port_of_loading_eta})


    @api.onchange('port_of_discharge_eta')
    def onchange_port_of_discharge_eta(self):
        if self.port_of_discharge_eta:
            # for si in self.env['freight.website.si'].search([('booking_ref', '=', self._origin.id),]):
            #     si.write({'port_of_discharge_eta': self.port_of_loading_eta})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self._origin.id),]):
                bl.write({'port_of_discharge_eta': self.port_of_loading_eta})


    # @api.onchange('booking_date_time')
    # def onchange_booking_date_time(self):
    #     if self.booking_date_time:
    #         for bl in self.env['freight.bol'].search([('booking_ref', '=', self._origin.id),]):
    #             bl.write({'date_of_issue': self.booking_date_time})
        ...
        if self.sale_order_template_id:
            if not self._origin or not self._origin.id:
                raise exceptions.ValidationError(
                    _("Please save the booking before selecting a Sales Quotation Template")
                )
        ...
        for line in self.sale_order_template_id.sale_order_template_line_ids:
            if line.product_id:
                ...
                cost_profit_line = cost_profit_obj.create({
                    ...
                    'profit_currency': line.currency_id.id,
                    'cost_price': line.cost_price or 0.0,
                    'cost_currency': line.cost_currency.id,
...
class FreightOperationLine(models.Model):
    ...
    bl_id = fields.Many2one('freight.bol', string='Charge Line', copy=False)
    ...
    def write(self, vals):
        ...
        if 'packages_no' in vals:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self.operation_id.id),]):
                for line in si.fcl_line_ids:
                    if line.container_no == self.container_no:
                        line.write({'packages_no': vals['packages_no']})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self.operation_id.id),]):
                for line in bl.cargo_line_ids:
                    if line.container_no == self.container_no:
                        line.write({'packages_no_value' : vals['packages_no']})
        if 'packages_no_uom' in vals:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self.operation_id.id),]):
                for line in si.fcl_line_ids:
                    if line.container_no == self.container_no:
                        line.write({'packages_no_uom': vals['packages_no_uom']})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self.operation_id.id),]):
                for line in bl.cargo_line_ids:
                    if line.container_no == self.container_no:
                        line.write({'packages_no_uom': vals['packages_no_uom']})
        if 'exp_net_weight' in vals:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self.operation_id.id),]):
                for line in si.fcl_line_ids:
                    if line.container_no == self.container_no:
                        line.write({'exp_net_weight': vals['exp_net_weight']})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self.operation_id.id),]):
                for line in bl.cargo_line_ids:
                    if line.container_no == self.container_no:
                        line.write({'exp_net_weight': vals['exp_net_weight']})
        if 'exp_gross_weight' in vals:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self.operation_id.id),]):
                for line in si.fcl_line_ids:
                    if line.container_no == self.container_no:
                        line.write({'exp_gross_weight': vals['exp_gross_weight']})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self.operation_id.id),]):
                for line in bl.cargo_line_ids:
                    if line.container_no == self.container_no:
                        line.write({'exp_gross_weight': vals['exp_gross_weight']})
        if 'exp_vol' in vals:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self.operation_id.id),]):
                for line in si.fcl_line_ids:
                    if line.container_no == self.container_no:
                        line.write({'exp_vol': vals['exp_vol']})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self.operation_id.id),]):
                for line in bl.cargo_line_ids:
                    if line.container_no == self.container_no:
                        line.write({'exp_vol': vals['exp_vol']})
...
class FreightOperationLine2(models.Model):
    ...
    def _onchange_container_no(self):
        ...
        if self.operation_id2.lcl_pcs:
            vals['packages_no'] = self.operation_id2.lcl_pcs
        if self.operation_id2.lcl_weight:
            vals['exp_gross_weight'] = self.operation_id2.lcl_weight
        if self.operation_id2.lcl_volume:
            vals['exp_vol'] = self.operation_id2.lcl_volume
        self.update(vals)

    @api.onchange('container_product_name')
    def _onchange_container_product_name(self):
        vals = {}
        if self.operation_id2.lcl_pcs and not self.packages_no:
            vals['packages_no'] = self.operation_id2.lcl_pcs
        if self.operation_id2.lcl_weight and not self.exp_gross_weight:
            vals['exp_gross_weight'] = self.operation_id2.weight
        if self.operation_id2.lcl_volume and not self.exp_vol:
            vals['exp_vol'] = self.operation_id2.lcl_volume
        self.update(vals)
    ...
    def write(self, vals):
        ...
        if 'packages_no' in vals:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self.operation_id2.id),]):
                if si.lcl_line_ids:
                    si.lcl_line_ids[0].write({'packages_no': vals['packages_no']})
        ...
        if 'packages_no_uom' in vals:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self.operation_id2.id),]):
                if si.lcl_line_ids:
                    si.lcl_line_ids[0].write({'packages_no_uom': vals['packages_no_uom']})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self.operation_id2.id),]):
                if bl.cargo_line_ids:
                    bl.cargo_line_ids[0].write({'packages_no_uom': vals['packages_no_uom']})
        if 'exp_net_weight' in vals:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self.operation_id2.id),]):
                if si.lcl_line_ids:
                    si.lcl_line_ids[0].write({'exp_net_weight': vals['exp_net_weight']})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self.operation_id2.id),]):
                if bl.cargo_line_ids:
                    bl.cargo_line_ids[0].write({'exp_net_weight': vals['exp_net_weight']})
        if 'exp_gross_weight' in vals:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self.operation_id2.id),]):
                if si.lcl_line_ids:
                    si.lcl_line_ids[0].write({'exp_gross_weight': vals['exp_gross_weight']})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self.operation_id2.id),]):
                if bl.cargo_line_ids:
                    bl.cargo_line_ids[0].write({'exp_gross_weight': vals['exp_gross_weight']})
        if 'exp_vol' in vals:
            for si in self.env['freight.website.si'].search([('booking_ref', '=', self.operation_id2.id),]):
                if si.lcl_line_ids:
                    si.lcl_line_ids[0].write({'exp_vol': vals['exp_vol']})
            for bl in self.env['freight.bol'].search([('booking_ref', '=', self.operation_id2.id),]):
                if bl.cargo_line_ids:
                    bl.cargo_line_ids[0].write({'exp_vol': vals['exp_vol']})
...
class CostProfit(models.Model):
    ...
    vendor_bill_id = fields.Many2one('account.invoice', string="Vendor Bill" , copy=False)
    vendor_bill_ids = fields.Many2many('account.invoice', string="Vendor Bill List", copy=False)
    cost_currency = fields.Many2one('res.currency', string="Curr", default=lambda self: self.env.user.company_id.currency_id.id,
                                    track_visibility='onchange')
    ...
    def _set_access_for_invoiced(self):
        if self.env['res.users'].has_group('account.group_account_invoice'):
