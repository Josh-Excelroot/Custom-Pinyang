...
class CustomerPortal(CustomerPortal):
    ...
    @http.route(
        ["/my/si", "/my/si/page/<int:page>"], type="http", auth="user", website=True
    )
    def my_si(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        sis = request.env["freight.website.si"].search(
    ...
    def home_page(self, si_id=None, **kw):
        si_count = request.env["freight.website.si"].search_count(
            [("si_status", "in", ["01", "02"])]
        )
        return request.render("sci_goexcel_freight.home_page", {"si_count": si_count})
    ...
    # Update a SI
    @http.route(
        ["/my/si/process"], type="http", auth="user", methods=["POST"], website=True
    )
    def si_update(self, **post):
        ...
        si_val = {
            ...
            "port_of_loading_input": request.params.get("port_of_loading"),
            "port_of_discharge_input": request.params.get("port_of_discharge"),
    ...
    @http.route(
        ["/my/si/delete_fcl_line/<int:si_id>/<int:line_id>"],
        type="http",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        ["/my/si/delete_lcl_line/<int:si_id>/<int:line_id>"],
        type="http",
        auth="user",
        website=True,
    )
    ...
    # When click done
    @http.route(
        ["/my/si/done"], type="http", auth="user", methods=["POST"], website=True
    )
    ...
    @http.route(
        ["/my/si/add_line"], type="http", auth="user", methods=["POST"], website=True
    )
    def si_add_line(self, **post):
        ...
        if si.cargo_type == "fcl":
            ...
            si_line = si_line_obj.create(
                {
                    "container_product_name": request.params.get(
    ...
    @http.route(
        ["/my/si/edit_line"], type="http", auth="user", methods=["POST"], website=True
    )
    def si_edit_line(self, **post):
        ...
        if si.cargo_type == "fcl":
            ...
            for line in si.fcl_line_ids:
                if line.id == line_id:
                    line.container_product_name = request.params.get(
                        "container_product_name"
                    )
