...
class InsFinancialReportXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, data):
        ...
        if data['form']['debit_credit'] != 1:
            ...
            self.sheet.write_string(self.row_pos, 0, _('Name'),
                                    self.format_header)
            ####Canon#####Start#####
            if data['form']['group_by'] != 'no_grouping':
                for i in data['form']['month_list']:
                    ...
                    if data['form']['show_percent_margin']:
                        self.sheet.write_string(self.row_pos, x, 'Marg %', self.format_header)
                        x = x + 1
                    # first period will not have perf %
                    if data['form']['month_list'][0] != i and data['form']['show_percent_performance']:
                        self.sheet.write_string(self.row_pos, x, 'Perf %', self.format_header)
                        x = x + 1
                ...
                if data['form']['enable_filter']:
                    ...
                    tmp += 1
                    if data['form']['show_percent_margin']:
                        self.sheet.write_string(self.row_pos, tmp, _('Marg %'),
                                                self.format_header)
                        tmp += 1
                ...
                if data['form']['show_percent_performance']:
                    self.sheet.write_string(self.row_pos, tmp, _('Perf %'),
                                            self.format_header)
            ...
            for a in data['report_lines'][1:]:  # [1:] because do not want to put the name of report here
                ...
                if data['form']['group_by'] != 'no_grouping':
                    for i in data['form']['month_list']:
                        ...
                        x = x + 1
                        if data['form']['show_percent_margin']:
                            self.sheet.write_string(self.row_pos, x, a.get('balance_margin_month').get(i), tmp_style_num)
                            x = x + 1
                        # first period will not have perf %
                        if data['form']['month_list'][0] != i and data['form']['show_percent_performance']:
                            self.sheet.write_string(self.row_pos, x, a.get('performance_percent').get(i), tmp_style_num)
                            x = x + 1
                    ...
                    if data['form']['show_percent_performance']:
                        self.sheet.write_string(self.row_pos, tmp,
                            a.get('performance_percent'), tmp_style_num)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        if not record:
            return False
        data = record.with_context(from_js=record.xlsx_from_js).get_report_values()
