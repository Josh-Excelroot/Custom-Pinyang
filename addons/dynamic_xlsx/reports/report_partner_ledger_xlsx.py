    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        if filter.get('include_details', False):
            ...
            self.sheet.write_string(self.row_pos, 5, _('Entry Label'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 6, _('Analytic Account'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 7, _('Analytic Tag'),
                                    self.format_header)
            ...
            self.sheet.merge_range(self.row_pos, 0, self.row_pos, 7, _('Partner'), self.format_header)
            self.sheet.write_string(self.row_pos, 8, _('Debit'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 9, _('Credit'),
                                    self.format_header)
        ...
        if acc_lines:
            for line in acc_lines:
                self.row_pos += 1
                self.sheet.merge_range(self.row_pos, 0, self.row_pos, 7, acc_lines[line].get('name'), self.line_header)
                self.sheet.write_number(self.row_pos, 8, float(acc_lines[line].get('debit')), self.line_header)
                self.sheet.write_number(self.row_pos, 9, float(acc_lines[line].get('credit')), self.line_header)
                self.sheet.write_number(self.row_pos, 10, float(acc_lines[line].get('balance')), self.line_header)

                if filter.get('include_details', False):

                    count, offset, sub_lines = self.record.build_detailed_move_lines(offset=0, partner=acc_lines[line]['id'],
                                                                                     fetch_range=1000000)

                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            self.row_pos += 1
                            self.sheet.write_string(self.row_pos, 7, sub_line.get('move_name'),
                                                    self.line_header_light_initial)
                            self.sheet.write_number(self.row_pos, 8, float(sub_line.get('debit')),
                                                    self.line_header_light_initial)
                            self.sheet.write_number(self.row_pos, 9, float(sub_line.get('credit')),
                                                    self.line_header_light_initial)
                            self.sheet.write_number(self.row_pos, 10, float(sub_line.get('balance')),
                                                    self.line_header_light_initial)
                            ...
                            self.sheet.write_string(self.row_pos, 5, sub_line.get('lname') or '',
                                                    self.line_header_light)
                            self.sheet.write_string(self.row_pos, 6, sub_line.get('analytic_account') or '',
                                                    self.line_header_light)
                            self.sheet.write_string(self.row_pos, 7, sub_line.get('analytic_tag') or '',
                                                    self.line_header_light)
                            ...
                            self.sheet.write_number(self.row_pos, 8, float(acc_lines[line].get('debit')),
                                                    self.line_header_light_ending)
                            self.sheet.write_number(self.row_pos, 9, float(acc_lines[line].get('credit')),
                                                    self.line_header_light_ending)
                            self.sheet.write_number(self.row_pos, 10, float(acc_lines[line].get('balance')),
                                                    self.line_header_light_ending)
