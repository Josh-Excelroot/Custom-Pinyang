    ...
    def prepare_report_contents(self, data, acc_lines, filter, final_balance):
        ...
        if acc_lines:
            for line in acc_lines:
                self.row_pos += 1
                # Rajeel | Analytic Report from journal items | 29/03/2023
                self.sheet.merge_range(self.row_pos, 0, self.row_pos, 5, line['name'], self.line_header_left)
                self.sheet.merge_range(self.row_pos, 6, self.row_pos, 10, 'Total Profit: '+str(line['balance']), self.line_header_right)
                for item in line['lines']:
                    self.row_pos += 1
                    self.sheet.write_string(self.row_pos, 6, item.get('reference', '') or '', self.line_header)
                    self.sheet.write_string(self.row_pos, 7, item.get('partner', '') or '', self.line_header)
                    self.sheet.write_number(self.row_pos, 8, float(item.get('credit', 0.0)) or 0.0,
                                            self.line_header)
                    self.sheet.write_number(self.row_pos, 9, float(item.get('debit', 0.0)) or 0.0,
                                            self.line_header)
                    self.sheet.write_number(self.row_pos, 10, float(item.get('balance', 0.0)) or 0.0, self.line_header)
                # Rajeel | END
            self.row_pos += 1
