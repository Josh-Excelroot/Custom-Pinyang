...
class InsTrialBalanceXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.language_id = self.env['res.lang'].search([('code', '=', lang)], limit=1)
        ...
        if record:
            ...
            # TS - update header (must be below filters above)
            self.sheet.merge_range(0, 0, 0, 10, data[0]['company_id'][1] + ' - ' + 'TRIAL BALANCE'
                                   + ' AS OF ' + filters['date_to'].strftime('%d/%m/%Y'), self.format_title)
            # Filter section
