...
class InsGeneralLedgerXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        if filter.get('include_details', False):
            ...
            self.sheet.merge_range(self.row_pos, 2, self.row_pos, 7, _('Account'), self.format_header)
            self.sheet.write_string(self.row_pos, 8, _('Debit'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 9, _('Credit'),
                                    self.format_header)
        ...
        if acc_lines:
            for line in acc_lines:
                self.row_pos += 1
                self.sheet.merge_range(self.row_pos, 0, self.row_pos, 7, '            ' + acc_lines[line].get('code') + ' - ' + acc_lines[line].get('name'), self.line_header_left)
