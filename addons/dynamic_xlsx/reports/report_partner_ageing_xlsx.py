...
class InsPartnerAgeingXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, filter):
        ...
        if filter:
            ...
            self.sheet_2.write_string(
                self.row_pos_2, 0, _("As on Date"), self.format_header
            )
            ...
            self.row_pos_2 += 1
            self.sheet_2.write_string(
                self.row_pos_2, 0, _("Partners"), self.format_header
            )
            ...
            self.row_pos_2 += 1
            self.sheet_2.write_string(
                self.row_pos_2, 0, _("Partner Tag"), self.format_header
            )
            ...
            self.row_pos_2 += 1
            self.sheet_2.write_string(
                self.row_pos_2, 0, _("Currency"), self.format_header
            )
            self.sheet_2.write_string(
                self.row_pos_2,
                1,
                filter.get("currency") if filter.get("currency") else "",
                self.content_header,
            )

            self.row_pos_2 += 1
            self.sheet_2.write_string(
                self.row_pos_2, 0, _("Sales Person"), self.format_header
            )
            self.sheet_2.write_string(
                self.row_pos_2,
                1,
                filter.get("salesperson") if filter.get("salesperson") else "",
                self.content_header,
            )

    def prepare_report_contents(
        self, data, period_dict, period_list, ageing_lines, filter
    ):
        data = data[0]
        self.row_pos += 3
        ...
        if self.record.include_details:
            ...
            self.sheet.write_string(self.row_pos, 1, _("Reference"), self.format_header)
            self.sheet.write_string(self.row_pos, 2, _("Due Date"), self.format_header)
            self.sheet.write_string(self.row_pos, 3, _("Payment Term"), self.format_header)
            self.sheet.write_string(self.row_pos, 4, _("Aging"), self.format_header)
            self.sheet.write_string(self.row_pos, 5, _("Journal"), self.format_header)
            self.sheet.write_string(self.row_pos, 6, _("Account"), self.format_header)
        else:
            self.sheet.merge_range(
                self.row_pos, 0, self.row_pos, 6, _("Partner"), self.format_header
            )
        k = 7
        for period in period_list:
            self.sheet.write_string(
                self.row_pos, k, str(period), self.format_header_period
            )
            k += 1
        ...
        if ageing_lines:
            for line in ageing_lines:
                ...
                self.row_pos += 1
                self.sheet.write_string(
                    self.row_pos, 6, "", self.line_header_light_period
                )
                self.sheet.write_string(
                    self.row_pos, 7, "", self.line_header_light_period
                )
                self.sheet.write_string(
                    self.row_pos, 8, "", self.line_header_light_period
                )
                self.sheet.write_string(
                    self.row_pos, 9, "", self.line_header_light_period
                )
                self.sheet.write_string(
                    self.row_pos, 10, "", self.line_header_light_period
                )
                self.sheet.write_string(
                    self.row_pos, 11, "", self.line_header_light_period
                )
                self.sheet.write_string(
                    self.row_pos, 12, "", self.line_header_light_period
                )
                self.sheet.write_string(
                    self.row_pos, 13, "", self.line_header_light_period
                )
                self.sheet.write_string(
                    self.row_pos, 14, "", self.line_header_light_period
                )
                ...
                if line != "Total":
                    self.sheet.merge_range(
                        ...
                        self.row_pos,
                        6,
                        ageing_lines[line].get("partner_name"),
                        self.line_header,
                    )
                else:
                    self.sheet.merge_range(
                ...
                for period in period_list:
                    if line != "Total":
                        self.sheet.write_number(
                            self.row_pos,
                            k,
                            ageing_lines[line].get(period, 0.0),
                            self.line_header,
                        )
                    else:
                        self.sheet.write_number(
                ...
                if line != "Total":
                    self.sheet.write_number(
                        self.row_pos, k, ageing_lines[line]["total"], self.line_header
                    )
                else:
                    self.sheet.write_number(
                        self.row_pos,
                        k,
                        ageing_lines[line]["total"],
                        self.line_header_total,
                    )
                ...
                if self.record.include_details:
                    if line != "Total":
                        (
                            ...
                            sub_lines,
                            period_list,
                        ) = self.record.process_detailed_data(
                            partner=line, fetch_range=1000000
                        )
                        for sub_line in sub_lines:
                            ...
                            self.sheet.write_string(
                                self.row_pos,
                                0,
                                sub_line.get("move_name") or "",
                                self.line_header_light,
                            )
                            self.sheet.write_string(
                                self.row_pos,
                                1,
                                sub_line.get("reference") or "",
                                self.line_header_light,
                            )
                            date = self.convert_to_date(
                                sub_line.get("date_maturity") or sub_line.get("date")
                            )
                            self.sheet.write_datetime(
                                self.row_pos, 2, date, self.line_header_light_date
                            )
                            self.sheet.write_string(
                                self.row_pos,
                                3,
                                sub_line.get("journal_name"),
                                self.line_header_light,
                            )
                            self.sheet.write_string(
                                self.row_pos,
                                4,
                                sub_line.get("payment_term"),
                                self.line_header_light,
                            )
                            self.sheet.write_string(
                                self.row_pos,
                                5,
                                sub_line.get("date_invoice"),
                                self.line_header_light,
                            )
                            self.sheet.write_string(
                                self.row_pos,
                                6,
                                sub_line.get("account_name") or "",
                                self.line_header_light,
                            )
                            ...
                            self.sheet.write_number(
                                self.row_pos,
                                7,
                                float(sub_line.get("range_0")),
                                self.line_header_light_period,
                            )
                            self.sheet.write_number(
                                self.row_pos,
                                8,
                                float(sub_line.get("range_1")),
                                self.line_header_light_period,
                            )
                            self.sheet.write_number(
                                self.row_pos,
                                9,
                                float(sub_line.get("range_2")),
                                self.line_header_light_period,
                            )
                            self.sheet.write_number(
                                self.row_pos,
                                10,
                                float(sub_line.get("range_3")),
                                self.line_header_light_period,
                            )
                            self.sheet.write_number(
                                self.row_pos,
                                11,
                                float(sub_line.get("range_4")),
                                self.line_header_light_period,
                            )
                            self.sheet.write_number(
                                self.row_pos,
                                12,
                                float(sub_line.get("range_5")),
                                self.line_header_light_period,
                            )
                            self.sheet.write_number(
                                self.row_pos,
                                13,
                                float(sub_line.get("range_6")),
                                self.line_header_light_period,
                            )
                            self.sheet.write_string(
                                self.row_pos, 14, "", self.line_header_light_period
    ...
    def _format_float_and_dates(self, currency_id, lang_id):
        ...
        self.content_header_date.num_format = DATE_DICT.get(
            lang_id.date_format, "dd/mm/yyyy"
        )
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.sheet.set_column(12, 12, 15)
        ...
        if record:
            ...
            self.sheet.merge_range(
                ...
                0,
                11,
                "Partner Ageing" + " - " + data[0]["company_id"][1],
                self.format_title,
            )
            ...
            self.prepare_report_contents(
                data, period_dict, period_list, ageing_lines, filters
            )
